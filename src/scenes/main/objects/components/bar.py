...
class BarComponent:
    ...
    def __init__(self, start: int, max: int, color: tuple = DEFAULT_BAR_COLOR):
        self.value = start
        if start > max:
            self.value = max
        self.max = max
    ...
    def set_value(self, value: int):
        self.value = value
        logging.debug(
            f"Установленое значение шкалы: {self.value} / {self.max}"
        )
    ...
    def draw(self, screen: pygame.Surface, size, local_position, camera):
        ...
        full_rect = pygame.Rect(
            local_position.x, local_position.y, border, size.y * camera.zoom
        )
