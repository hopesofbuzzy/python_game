...
class UpgradeManager:
    ...
    def __init__(
        ...
        event_bus.subscribe(
            "on_requested_upgrade_dialog", self.open_upgrade_dialog
        )
    ...
    def plant_level_up(
        ...
        new_plant = self.plant_factory.create_plant(
            target_plant_name, global_pos_centred, tile_pos
        )
        self.gamemap.get(MapLevelDataComponent).remove_plant(tile_pos)
        self.gamemap.get(MapLevelDataComponent).add_plant(
            tuple(new_plant.get(DataComponent).tile_pos)
        )
        plant.free()
        # Открытие нового окна улучшения.
        self.open_upgrade_dialog(EventFlow(), new_plant)
    ...
    def open_upgrade_dialog(self, _event: EventFlow, plant: BasePlant):
        """Открытие диалогового окна улучшения растения."""
        self.close_all_upgrade_dialogs()
        plant_name = plant.get(DataComponent).name
        dialog = (
            DialogBuilder(self.add_object, self.ui_factory, self.event_bus)
            .with_dialog(Vector2(0, 0), UPGRADE_DIALOG_SIZE, plant)
            .with_text(PLANTS_UPGRADE_DESCRIPTIONS[plant_name], 18)
            .with_button(UPGRADE_BUTTON_TEXT, UPGRADE_BUTTON_FONT_SIZE, plant)
            .build()
        )
