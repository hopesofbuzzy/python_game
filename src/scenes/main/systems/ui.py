...
class UIManager:
    ...
    def __init__(
        ...
        self.event_bus.subscribe(
            "on_tooltip_hide_requested", self.hide_tooltip
        )
        self.tooltip = None
    ...
    def build_stats(self):
        ...
        self.currency.on_suns_update.subscribe(
            lambda s: self.suns_text.get(TextRenderComponent).set_text(
                f"Солнышки: {s}"
            )
        )
        ...
        for slot in inventory_slots:
            ...
            image.get(ClickHandlerComponent).on_button_pressed.subscribe(
                lambda clicked_slot: self.event_bus.fire(
                    "on_inventory_changed_slot", clicked_slot
                )
            )
